...
**Rule fit:** All reasoning runs as a local model on the Snapdragon NPU (Gemma 3n / Gemma 2B via `flutter_gemma`). Internet is allowed and used only to pull in live data — never for LLM inference.

---

## Core — live on stage, all reasoning on the NPU

1. **Scene description** — point the camera, Gemma 3n on the NPU describes what's in front of you, spoken aloud.
...
4. **Voice-first, eyes-free** — Whisper Tiny for input, TTS for every response. No screen touch needed.
