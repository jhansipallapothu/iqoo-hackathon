...
| On-device language model | Implemented; automated tested (analyze + release build only, no device run) | 2026-09-07: wired `flutter_gemma` (LiteRT-LM engine) running Qwen3 0.6B (public model, no HF token needed) into `AiService.explain()`; `on_device_llm_service.dart` is no longer a stub. `on_device_llm` feature flag flipped to true. `minSdkVersion` raised to 30 (required by the plugin's LiteRT inference — narrows device compatibility below Android 11). `flutter analyze` and `flutter build apk --release` both pass; the model itself has never run on a physical phone from this session (no device attached). First launch needs network to download the ~586 MB model once; only offline afterward. Cloud Gemini remains the fallback on any on-device init/generation failure or timeout. | Run on the loaner iQOO phone: confirm the download completes, confirm `PreferredBackend.npu` actually engages (vs. silently falling back to GPU/CPU), measure latency/RAM/heat, and confirm the OCR-fast-path + template-fallback safety net still holds if on-device stalls. |
| English-only localization | Implemented scope decision | English is the supported experience. Tamil strings, config fields (`tamil_support`, `wake_word_ta`), and the `isTamil` branches were removed from the codebase on 2026-09-07 (previously dead code hard-wired off). This limits who can benefit. | Keep claims explicit; discuss future languages only after English reliability is validated. |
